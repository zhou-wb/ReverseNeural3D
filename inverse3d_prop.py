...
class InversePropagation(nn.Module):
    def __init__(self, inverse_network_config, **kwargs):
        ...
        elif self.config == 'uformer_asm_dpac':
            self.target_uformer = Uformer(embed_dim=32, # 可以换成64，显存占用暴涨
                                          depths=[2, 2, 2, 2, 2, 2, 2, 2, 2],  # 【4, 4, 4, 4, 4, 4, 4, 4, 4】也可以
                                          in_chans=2,  # out_channel=in_chans 输出通道数
                                          img_size=kwargs['image_res'], 
                                          dd_in=len(kwargs['prop_dists_from_wrp']), 
                                          win_size=8, # 窗口大小
                                          mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True, shift_flag=False)
            self.asm_dpac = DPAC(kwargs['prop_dist'], kwargs['wavelength'], kwargs['feature_size'], prop_model='ASM', 
                                 propagator=propagation_ASM, device=kwargs['device'])
            self.forward = self.inverse_Uformer_ASM_DPAC
        else:
    ...
    def inverse_Uformer_ASM_DPAC(self, masked_imgs):
        ########## phase generated by Uformer+ASM+DPAC ###############
        mid_amp_phase = self.target_uformer(masked_imgs)
        mid_amp = mid_amp_phase[:,0:1,:,:]
        mid_phase = mid_amp_phase[:,1:2,:,:]
        _, slm_phase = self.asm_dpac(mid_amp, mid_phase)
        ##########################################################
    ...
    def configoptimizer(self, props, lr):
        if self.config == 'cnn_only':
            ...
            return torch.optim.AdamW(reverse_prop.parameters(), lr=lr, betas=(0.9, 0.999)) # 那篇论文用的是AdamW
