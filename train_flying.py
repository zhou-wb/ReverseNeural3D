...
for i in range(max_epoch):
    ...
    for imgs_masks_id in train_dataloader:
        ...
        # slm_phase = target_cnn(masked_imgs)
        ##########################################################
        
        ########## phase generated by CNN+ASM+DPAC ###############
        # mid_amp_phase = target_cnn(masked_imgs)
        ...
        # slm_field = inverse_asm(mid_field)
        
        # slm_phase = slm_cnn(torch.cat([slm_field.abs(), slm_field.angle()], dim=1))
        ##########################################################
