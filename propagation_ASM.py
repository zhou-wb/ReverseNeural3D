...
import torch
import numpy as np
import utils_2d.utils_2d as utils_2d
import torch.fft
...
def propagation_ASM(u_in, feature_size, wavelength, z, linear_conv=True,
    ...
    if linear_conv:
        ...
        u_in = utils_2d.pad_image(u_in, conv_size, padval=padval, stacked_complex=False)
    ...
    if precomped_H is None:
        ...
        H_real, H_imag = utils_2d.polar_to_rect(H_filter.to(u_in.device), H_exp)

        H = torch.stack((H_real, H_imag), 4)
        H = utils_2d.ifftshift(H)
    ...
    U1 = torch.fft.fftn(utils_2d.ifftshift(u_in), dim=(-2, -1), norm='ortho')

    U2 = H * U1

    u_out = utils_2d.fftshift(torch.fft.ifftn(U2, dim=(-2, -1), norm='ortho'))

    if linear_conv:
        # return utils.crop_image(u_out, input_resolution) # using stacked version
        return utils_2d.crop_image(u_out, input_resolution, pytorch=True, stacked_complex=False)  # using complex tensor
    else:
        return u_out